...
/**
 * Favourites Section Component
 * Displays favourite/featured games in a horizontal scrollable layout
 */
...
export default function FavouritesSection() {
	...
	const fetchFavouriteGames = async () => {
		try {
			...
			const res = await casinoService.getGames({ limit: 20, sort: '-stats.totalPlays' });
			if (res.success) {
				setGames(res.data);
			}
		} catch (error) {
			console.error('Failed to fetch favourite games:', error);
		} finally {
			setLoading(false);
		}
	};
			...
			<div className="mx-0 md:mx-8 mt-0 p-2">
				...
				<div className="flex items-center justify-between mb-4 py-[11px] md:py-0">
					<div className="flex items-center gap-3">
						...
						<h3 className="font-extrabold text-[17.2px] md:text-[17.2] text-black">Favourites</h3>
					</div>
						...
						<div className="flex gap-3 px-2">
							{games.map((game) => {
								...
								return (
									<button
										key={game._id}
										className="bg-white rounded-lg border border-gray-200 overflow-hidden shrink-0 hover:shadow-lg transition-shadow relative group w-50 h-40 sm:w-66 h-30"
										type="button"
									...
									>
										...
										<div className="p-2 bg-white">
											<p className="text-xs font-medium text-gray-800 truncate text-center">
												{game.gameName}
											</p>
										</div>
									</button>

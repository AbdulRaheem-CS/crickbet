...
module.exports.launchGame = asyncHandler(async (req, res) => {
  ...
  // For testing, if localhost/private, use a public IP to pass provider validation
  // PG Soft often blocks 127.0.0.1. Using a generic public IP (Google DNS) for testing.
  ...
  try {
    ...
    console.log(`[Casino] Pre-launch check for game ID: ${game._id}`);
    console.log(`[Casino] Game details: Name=${game.gameName}, Code=${game.gameCode}, Product=${game.productCode}, Type=${game.gameType}`);
    console.log(`[Casino] User details: Username=${user.username}, Wallet=${JSON.stringify(user.wallet)}`);

    const operatorCurrency = await gscService.getProductCurrency(game.productCode, game.gameType);
    console.log(`[Casino] Resolved operator currency: ${operatorCurrency}`);
    ...
    console.log(`[Casino] Launch result:`, JSON.stringify(result, null, 2));

...
exports.getProductCurrency = async (productCode, gameType) => {
  console.log(`[GSC+] getProductCurrency called for ${productCode}, ${gameType}`);
  const now = Date.now();
  ...
  if (!_productCurrencyCache || (now - _productCurrencyCacheTime) > CACHE_TTL) {
    try {
      ...
      console.log(`[GSC+] Fetching products from: ${url}`);

      const response = await fetch(url, { headers: { 'Content-Type': 'application/json' } });
      console.log(`[GSC+] Product list response status: ${response.status}`);
...
const makeRequest = async (method, path, data = {}) => {
  ...
  try {
    ...
    console.log(`[GSC+] Response code: ${result.Code || result.code}, Message: ${result.Message || result.message}`);
    // console.log(`[GSC+] Full response body:`, JSON.stringify(result)); // Uncomment for full debug
